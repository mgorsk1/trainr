...
def get_router(handler: SystemSettingsHandlerModel):
    ...
    async def get_mode_state() -> SystemSettingInfoApiModel:
    ...
    async def set_mode(mode: SystemSettingInputApiModel) -> SystemSettingInfoApiModel:
        handler.set_mode(mode.setting_value)

        data = handler.get_state()

        return SystemSettingInfoApiModel(setting_value=data.setting_value)
